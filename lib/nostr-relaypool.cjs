...
module.exports = __toCommonJS(nostr_relaypool_ts_exports);

// merge-similar-filters.ts
var import_safe_stable_stringify = require("safe-stable-stringify");
function indexForFilter(filter, key) {
  let new_filter = { ...filter };
  delete new_filter[key];
  return key + (0, import_safe_stable_stringify.stringify)(new_filter);
}
...
var RelayC = class {
    ...
    if (data.length >= 1) {
      switch (data[0]) {
        case "EVENT":
          ...
          if ((0, import_nostr_tools.validateEvent)(event2) && this.openSubs[id] && (this.openSubs[id].skipVerification || (0, import_nostr_tools.verifySignature)(event2)) && (0, import_nostr_tools2.matchFilters)(this.openSubs[id].filters, event2)) {
            this.openSubs[id];
  ...
  async connectRelay() {
    return new Promise((resolve, reject) => {
      const ws = new import_isomorphic_ws.default(this.url);
      this.ws = ws;
...
var EventCache = class {
  ...
  #addEventToAuthorKindsByPubKey(event) {
    ...
    if (!kindsByPubKey) {
      ...
      if (!events) {
        ...
        if (event.kind === import_nostr_tools3.Kind.Metadata || event.kind === import_nostr_tools3.Kind.Contacts) {
  ...
  addEvent(event) {
    ...
    if (event.kind === import_nostr_tools3.Kind.Metadata) {
      this.metadataByPubKey.set(event.pubkey, event);
    }
    if (event.kind === import_nostr_tools3.Kind.Contacts) {
  ...
  #getCachedEventsByPubKeyWithUpdatedFilter(filter) {
    if (filter.noCache || !filter.authors || !filter.kinds || filter.kinds.find(
      (kind) => kind !== import_nostr_tools3.Kind.Contacts && kind !== import_nostr_tools3.Kind.Metadata
    ) !== void 0) {
      return void 0;
    }
    const authors = [];
    ...
    for (const author of filter.authors) {
      let contactEvent;
      if (filter.kinds.includes(import_nostr_tools3.Kind.Contacts)) {
      ...
      let metadataEvent;
      if (filter.kinds.includes(import_nostr_tools3.Kind.Metadata)) {
  ...
  metaData(cb, maxDelayms) {
    return this.relayPool.subscribeEventObject(
      [
        {
          authors: [this.pubkey],
          kinds: [import_nostr_tools4.Kind.Metadata]
        }
      ],
      this.relays,
      cb,
  ...
  followsPubkeys(cb, maxDelayms) {
    return this.relayPool.subscribeEventObject(
      [
        {
          authors: [this.pubkey],
          kinds: [import_nostr_tools4.Kind.Contacts]
        }
      ],
      this.relays,
      (event) => {
  ...
  sentAndRecievedDMs(cb, limit = 100, maxDelayms) {
    return this.relayPool.subscribe(
      [
        {
          authors: [this.pubkey],
          kinds: [import_nostr_tools4.Kind.EncryptedDirectMessage],
          limit
        },
        {
          "#p": [this.pubkey],
  ...
  text(cb, limit = 100, maxDelayms) {
    return this.relayPool.subscribe(
      [
        {
          authors: [this.pubkey],
          kinds: [import_nostr_tools4.Kind.Text],
          limit
        }
      ],
      this.relays,
...
function doNotEmitOlderEvents(onEvent) {
  ...
  return (event, afterEose, url) => {
    if (event.kind === import_nostr_tools5.Kind.Metadata || event.kind === import_nostr_tools5.Kind.Contacts) {
...
function matchOnEventFilters(onEvent, filters) {
  return (event, afterEose, url) => {
    for (let filter of filters) {
      if ((0, import_nostr_tools5.matchFilter)(filter, event)) {
        onEvent(event, afterEose, url);
        break;
      }
    }
